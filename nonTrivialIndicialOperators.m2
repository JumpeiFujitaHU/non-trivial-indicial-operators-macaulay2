...
nonTrivialIndicialOperators = (A,beta,w) -> (
  wmp = (-w) | w;
  d = numrows A;
  n = numcols A;
  D = makeWA(QQ[x_1..x_n]);
  I' = ideal(0_D);
  I_A = toricIdealPartials(A,D);
  InwI_A = inw(substitute(I_A, D), wmp);
  gbwI_A = flatten entries gens gbw(substitute(I_A, D), wmp);
  LeadTermsOfgbwI_A = for i from 0 to (#gbwI_A - 1) list inw(gbwI_A#i, wmp);
  NonLeadTermsOfgbwI_A = for i from 0 to (#gbwI_A - 1) list -(gbwI_A#i - LeadTermsOfgbwI_A#i);
  SM = standardPairs monomialIdeal substitute(InwI_A, QQ[dx_1..dx_n]);
  ...
  while #EMbetaList > 0 do (
    ...
    while i != d - #sigma do (
      ...
      h = sum OperationAstFn(terms(q*thetaa*g), LeadTermsOfgbwI_A, NonLeadTermsOfgbwI_A, wmp);
      inwh = inw(h, wmp);
      for t from 0 to n-1 do (
        ...
        if (((f * submatrix(substitute(A, QQ), , {t}))_(0,0))*q*thetaaplus1t - inwh) % sub(I_A, D) == 0_D
        then (T = append(T, t+1));
      );
      inwhg = inwh * g;
      r = inwhg % InwI_A;
      c = if (#(terms r) == #(terms inwh) and (r)%(inwh) == 0) then sub((flatten entries (coefficients (inwhg % InwI_A))#1)#0,QQ)/sub((flatten entries (coefficients (inwh % InwI_A))#1)#0,QQ) else ("doesntexist");
      while (class c === QQ and isMember(inwhg - c*inwh,InwI_A)) do (
        h' = sum OperationAstFn(terms(inwhg), LeadTermsOfgbwI_A, NonLeadTermsOfgbwI_A, wmp);
        h = h' - c*h;
        inwh = inw(h, wmp);
        inwhg = inwh * g;
        r = inwhg % InwI_A;
  ...
  DistI = ideal select(flatten entries gens distraction(I',QQ[θ_1..θ_n]), f -> f != 0);
  DistI
);